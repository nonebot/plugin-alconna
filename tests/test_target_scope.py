...
from nonebot.adapters.satori import Adapter as SatoriAdapter, Bot as SatoriBot
from nonebot.adapters.onebot.v11 import Adapter as Onebot11Adapter, Bot as Onebot11Bot, MessageSegment
from nonebot.adapters.onebot.v12 import Adapter as Onebot12Adapter, Bot as Onebot12Bot



@pytest.mark.asyncio()
async def test_bots(app: App):
    from nonebot_plugin_alconna import UniMessage, Target, SupportScope
    ...
    async with app.test_api() as ctx:
        ...
        satori_bot_cc = ctx.create_bot(base=SatoriBot, adapter=satori_adapter, self_id="2", platform="chronocat", info=None)
        ...
        onebot12_bot = ctx.create_bot(base=Onebot12Bot, adapter=onebot12_adapter, self_id="5", platform="qq", impl="mock")
        ...
        ctx.should_call_api(
            "message_create",
            {
                'channel_id': '456', 'content': 'test',
            }
        )
        await target2.send(UniMessage("test"))
        ctx.should_call_api(
            "post_messages",
            {'channel_id': '789', 'msg_id': '', 'event_id': None, 'content': 'test'}
        )
